...
const URL_ICON_SCSS = 'https://raw.githubusercontent.com/vorillaz/devicons/master/css/devicons.scss';
const URL_SVG_FONTS = 'https://raw.githubusercontent.com/vorillaz/devicons/master/fonts/devicons.svg';
const DEFAULT_OUTPUT = path.resolve(__dirname, '../../dist/data/devicons.json');

function extractIcons(options) {
  // less file for icon fonts
  request(URL_ICON_SCSS, (iconLessErr, iconLessRes, iconLessBody) => {
    if (!iconLessErr && iconLessRes.statusCode === 200) {
      ...
      lang.each(iconLessBody.split(/\n/), (line) => {
        ...
        // icon font pattern
        // (debian "\e67d")
        const match = /\((\s*[^\s]+)\s*"\\([0-9a-f]+)"\s*\)/.exec(line);
        if (match) {
          const name = match[1];

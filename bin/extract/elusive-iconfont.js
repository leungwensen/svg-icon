...
const URL_ICON_LESS = 'https://raw.githubusercontent.com/reduxframework/elusive-iconfont/master/less/variables.less';
const URL_SVG_FONTS = 'https://raw.githubusercontent.com/reduxframework/elusive-iconfont/master/fonts/Elusive-Icons.svg';
const DEFAULT_OUTPUT = path.resolve(__dirname, '../../dist/data/elusive-iconfont.json');
  ...
  request(URL_ICON_LESS, (iconLessErr, iconLessRes, iconLessBody) => {
    if (!iconLessErr && iconLessRes.statusCode === 200) {
      ...
      lang.each(iconLessBody.split(/\n/), (line) => {
        ...
        const match = /@el-var-([^\s:]+)\s*:\s*"\\([0-9a-f]+)"\s*;/.exec(line);
        if (match) {
          const name = match[1];

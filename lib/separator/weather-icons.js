...
module.exports = (source, target/* , options */) => {
  ...
  co(function *() {
      ...
      const fontPath = path.resolve(source, './font/weathericons-regular-webfont.svg');
      ...
      const lessPath = path.resolve(source, './css/weather-icons.css');
      const lessContent = yield readFile(lessPath, 'utf8');
      const iconList = [];
      const matchedHexes = [];
      const matched = lessContent.match(/\.wi-([^:]+):before\s*[^"]+"\\([0-9a-f]+)"/g);
      lang.each(matched, line => {
        if (!line) return;

        const match = /\.wi-([^:]+):before\s*[^"]+"\\([0-9a-f]+)"/.exec(line);
        if (match) {

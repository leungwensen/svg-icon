...
module.exports = (source, target/* , options */) => {
  ...
  co(function *() {
      ...
      const fontPath = path.resolve(source, './fonts/payment-webfont.svg');
      ...
      const lessPath = path.resolve(source, './style.css');
      const lessContent = yield readFile(lessPath, 'utf8');
      const iconList = [];
      const matchedHexes = [];
      const matched = lessContent.match(/\.pw-([^:]+):before\s*[^"]+"\\([0-9a-f]+)"/g);
      lang.each(matched, line => {
        if (!line) return;

        const match = /\.pw-([^:]+):before\s*[^"]+"\\([0-9a-f]+)"/.exec(line);
        if (match) {
      ...
      generateSvg(iconList, target, 256);
    }
  );
};

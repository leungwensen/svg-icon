...
module.exports = (source, target/* , options */) => {
  ...
  co(function *() {
      ...
      const fontPath = path.resolve(source, './fonts/metro.svg');
      ...
      const lessPath = path.resolve(source, './less/metro-icons-content.less');
      const lessContent = yield readFile(lessPath, 'utf8');
      const iconList = [];
      const matchedHexes = [];
      const matched = lessContent.match(/\.mif-([^:]+):before[^"]+"\\([0-9a-f]+)"/g);
      lang.each(matched, line => {
        if (!line) return;

        const match = /\.mif-([^:]+):before[^"]+"\\([0-9a-f]+)"/.exec(line);
        if (match) {

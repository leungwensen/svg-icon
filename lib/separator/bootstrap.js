...
module.exports = (source, target/* , options */) => {
  ...
  co(function *() {
      ...
      const fontPath = path.resolve(source, './fonts/glyphicons-halflings-regular.svg');
      ...
      // get font list from `./less/glyphicons.less`
      const lessPath = path.resolve(source, './less/glyphicons.less');
      ...
      lang.each(lessContent.split(/\n/), line => {
        ...
        const match = /\.glyphicon-([^\{\s]+)[^"]+"\\([0-9a-f]+)";/.exec(line);
        if (match) {

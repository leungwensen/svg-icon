...
module.exports = (svgStr, callback, svgoInitOpt, svgoFinalOpt) => {
  // NOTE init an Nightmare instance every session, for it will be destroy after .end()
  const nightmare = new Nightmare({
    show: false,
  });
  const svgoInit = svgoInitOpt ? new SVGO(svgoInitOpt) : DEFAULT_SVGO_INIT;
  const svgoFinal = svgoFinalOpt ? new SVGO(svgoFinalOpt) : DEFAULT_SVGO_FINAL;
